...
(
	...
	input                       rst,
	input[11:0]                 video_left_offset,//视频水平偏移
	input[11:0]                 video_top_offset,//视频垂直偏移
	input[11:0]                 video_width,//视频宽度
	input[11:0]                 video_height,//视频高度
	output reg                  read_req,           // Start reading a frame of data     
	input                       read_req_ack,       // Read request response
	(*mark_debug = "true"*)output                      read_en,            // Read data enable    debug测试
	input[DATA_WIDTH - 1:0]     read_data,          // Read data
	input                       timing_hs,    //行同步
	input                       timing_vs,    //场同步
	(*mark_debug = "true"*)input                       timing_de,//使能    
	(*mark_debug = "true"*)input[DATA_WIDTH - 1:0]     timing_data, //RGB数据
...
begin
	if(rst == 1'b1)
		read_en_r <= 1'b0;
	        ...
	        && pos_y >= video_top_offset && pos_y < video_top_offset + video_height)//图像显示区域
		read_en_r <= 1'b1;
	else
		read_en_r <= 1'b0;
end
...
begin
	if(rst == 1'b1)
		...
		vout_data_r <= read_data;//从ddr读到一帧数据
	else
